...
(function () {
  ...
  Firechat.prototype = {
    ...
    _setupDataEvents: function () {
      ...
      var usernameRef = this._usersOnlineRef.child(
    ...
    _onNotification: function (snapshot) {
      ...
      if (!notification.read) {
        if (
          notification.notificationType !== "suspension" ||
  ...
  Firechat.prototype.enterRoom = function (roomId) {
    ...
    self.getRoom(roomId, function (room) {
      ...
      if (self._user) {
        ...
        var presenceRef = self._firechatRef
          .child("room-users")
          .child(roomId)
          .child(self._userId)
          .child(self._sessionId);
        self._queuePresenceOperation(
          presenceRef,
          {
            id: self._userId,
            name: self._userName,
      ...
      self._roomRef.child(roomId).once(
        "value",
        function (snapshot) {
          self._messageRef
            .child(roomId)
            .limitToLast(self._options.numMaxMessages)
            .on(
              "child_added",
              function (snapshot) {
  ...
  Firechat.prototype.sendMessage = function (
    roomId,
    messageContent,
    messageType,
    cb
  ...
  Firechat.prototype.toggleUserMute = function (userId, cb) {
    ...
    self._userRef
      .child("muted")
      .child(userId)
      .transaction(function (isMuted) {
        return isMuted ? null : true;
      }, cb);
  };

  // Send a moderator notification to a specific user.
  Firechat.prototype.sendSuperuserNotification = function (
    userId,
    notificationType,
    data,
    cb
    ...
    var self = this,
      userNotificationsRef = self._firechatRef
        .child("users")
        .child(userId)
        .child("notifications");
  ...
  Firechat.prototype.inviteUser = function (userId, roomId) {
    var self = this,
      sendInvite = function () {
        var inviteRef = self._firechatRef
          .child("users")
          .child(userId)
          .child("invites")
          .push();
        ...
        inviteRef.on(
          "value",
          self._onFirechatInviteResponse,
          function () {},
          self
        );
      };
    ...
    self.getRoom(roomId, function (room) {
      if (room.type === "private") {
        var authorizedUserRef = self._roomRef
          .child(roomId)
    ...
    self._userRef
      ...
      .once(
        ...
        function (snapshot) {
          ...
          if (invite === null && cb) {
            return cb(
              new Error("acceptInvite(" + inviteId + "): invalid invite id")
            );
          } else {
  ...
  Firechat.prototype.getUsersByPrefix = function (
    prefix,
    startAt,
    endAt,
    limit,
    ...
    query.once("value", function (snapshot) {
      ...
      for (var userNameKey in usernames) {
        ...
        if (
          prefix.length > 0 &&
          userName.toLowerCase().indexOf(prefixLower) !== 0
        )
          continue;
