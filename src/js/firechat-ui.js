(function ($) {
  ...
  FirechatUI.prototype = {
    ...
    _onChatInvite: function (invitation) {
      var self = this;
      var template =
        FirechatDefaultTemplates["templates/prompt-invitation.html"];
      ...
      if (notification.notificationType === "warning") {
        ...
        var suspendedUntil = notification.data.suspendedUntil,
          secondsLeft = Math.round(
            (suspendedUntil - new Date().getTime()) / 1000
          ),
          timeLeft = "";
    ...
    var self = this,
      parseMessageVars = function (event) {
        var $this = $(this),
          ...
          userId = $('[data-message-id="' + messageId + '"]')
            .closest("[data-user-id]")
            .data("user-id"),
          roomId = $('[data-message-id="' + messageId + '"]')
      ...
      function (event) {
        ...
        self._chat.getUsersByRoom(roomId, function (users) {
          for (var username in users) {
            ...
            user.nameTrimmed = self.trimWithEllipsis(
              user.name,
              self.maxLengthUsernameDisplay
            );
            user.isMuted =
              self._user && self._user.muted && self._user.muted[user.id];
  ...
  FirechatUI.prototype._bindForUserSearch = function () {
    var self = this,
      ...
      userSearch = function (
        ...
        self._chat.getUsersByPrefix(
          prefix,
          startAt,
          endAt,
          self.maxUserSearchResults,
          function (users) {
            var numResults = 0,
              $prevBtn,
              $nextBtn,
            ...
            if ($controls) {
              $prevBtn = $controls.find(
                '[data-toggle="firechat-pagination-prev"]'
              );
              $nextBtn = $controls.find(
    ...
    $(document).delegate(
      ...
      function (event) {
        var $this = $(this),
          ...
          template =
            FirechatDefaultTemplates["templates/prompt-user-mute.html"];
  ...
  FirechatUI.prototype._bindForChatInvites = function () {
    var self = this,
      renderInvitePrompt = function (event) {
        var $this = $(this),
          ...
          userName = $this.closest("[data-user-name]").data("user-name"),
          template =
            FirechatDefaultTemplates["templates/prompt-invite-private.html"],
          $prompt;
    ...
    $(document).delegate(
      '[data-event="firechat-user-chat"]',
      "click",
      renderPrivateInvitePrompt
    );
    $(document).delegate(
  ...
  FirechatUI.prototype.attachTab = function (roomId, roomName) {
    ...
    $textarea.bind("keydown", function (e) {
      var message = self.trimWithEllipsis(
        $textarea.val(),
        self.maxLengthMessage
      );
    ...
    var tabListTemplate =
      FirechatDefaultTemplates["templates/tab-menu-item.html"];
  ...
  FirechatUI.prototype.showMessage = function (roomId, rawMessage) {
    ...
    message.message = self.trimWithEllipsis(
      message.message,
      self.maxLengthMessage
    );
    ...
    if ($messages) {
      ...
      if (
        $messages.scrollTop() /
          ($messages[0].scrollHeight - $messages[0].offsetHeight) >=
        0.95
      ) {
        // Pinned to bottom
        scrollToBottom = true;
  ...
  FirechatUI.prototype.trimWithEllipsis = function (str, length) {
    ...
    return length && str.length <= length
      ? str
  ...
  FirechatUI.prototype.promptCreateRoom = function () {
    var self = this;
    var template =
      FirechatDefaultTemplates["templates/prompt-create-room.html"];
